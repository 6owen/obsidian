...
<!-- input: 年份日记、集中素材目录、日记模板与 weekly / publish 属性。
...
172 个历史素材与 2026 年周记封面统一保存在 `obsidian/assets/`，另保留 1 张旧周记未引用原图，共 174 个文件；历史素材中的静态图经 Sharp 压缩，总体积由约 639 MB 降至约 141 MB；动图和视频保留原文件。
文章的实际图片与视频引用已改成本地相对路径，不再依赖旧图床。

年份目录中的正文和 `cover` 使用 `../../assets/文件名` 引用集中素材；例如 `![](../../assets/订婚了.webp)`。
Obsidian 新附件的默认保存位置为 `obsidian/assets`；位于日记根目录的笔记使用 `../assets/文件名`，归档到年份目录时更新相对路径。
